...
//		});
		pw.$(fid).one('load', function(e) {
			...
			var h = setInterval(function() {
				if (w.baidu) {// 等待加载完成，IE6下这地方总出问题
					clearInterval(h);
					//w.location.href = location.href+"&frame=true";
	...
	commonData : {// 针对测试文件的路径而不是UserAction的路径
		...
		"testdir" : '../../',
		datadir : (function() {
//			return location.href.split("/run.do")[0] + "/test/tools/data/";
			return location.pathname.substring(0,location.pathname.indexOf("exec")) + "view/test/tools/data/";
		})(),
		currentPath : function() {
			return location.pathname.substring(0,location.pathname.lastIndexOf("/")).replace("exec","view") + "/";
//			var params = location.search.substring(1).split('&');
//			for ( var i = 0; i < params.length; i++) {
//				var p = params[i];
//				if (p.split('=')[0] == 'case') {
//					var casepath = p.split('=')[1].split('.').join('/');
//					return location.href.split('/run.do')[0] + '/test/'
